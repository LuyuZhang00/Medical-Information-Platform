...
import com.barry.order.utils.ConstantPropertiesUtils;
import com.barry.order.utils.HttpClient;
...
public class WeixinServiceImpl implements WeixinService {
    ...
    public Map createNative(Long orderId) {
        try {
            Map payMap = (Map)
                    redisTemplate.opsForValue().get(orderId.toString());
            if (null != payMap) {
                return payMap;
            }
            //根据 id 获取订单信息
            ...
            //2、HTTPClient 来根据 URL 访问第三方接口并且传递参数
            ...
            if (null != resultMap.get("result_code")) {
                //微信支付二维码 2 小时过期，可采取 2 小时未支付取消订单
    ...
    public Boolean refund(Long orderId) {
        try {
            ...
            if (refundInfo.getRefundStatus().intValue() ==
                    RefundStatusEnum.REFUND.getStatus().intValue()) {
                return true;
            }
            Map<String, String> paramMap = new HashMap<>(8);
            paramMap.put("appid", ConstantPropertiesUtils.APPID); //公众账号 ID
            ...
            paramMap.put("out_refund_no", "tk" + paymentInfoQuery.getOutTradeNo()); // 商 户 退款单号
            //
